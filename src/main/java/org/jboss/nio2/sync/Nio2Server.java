...
public class Nio2Server extends Thread {
	...
	private static final Logger logger = Logger.getLogger(Nio2Server.class
			.getName());
	...
	public Nio2Server(int port) throws Exception {
		...
		this.serverSocketChannel = ServerSocketChannel.open().bind(
		...
		while (running) {
			try {
				...
				if (channel != null) {
					...
					Nio2ClientManager clientManager = new Nio2ClientManager(
							channel);
